...
export function useJobMatcher() {
  ...
  const refetchJobs = async () => {
    const lastMatchRequest = queryClient.getQueryData<MatchJobsRequest>([
      "lastMatchRequest",
    ]);
    const lastFilterRequest = queryClient.getQueryData<JobFilterRequest>([
      "lastFilterRequest",
    ]);
  ...
  return {
    ...
    error:
      matchMutation.error?.message || filterMutation.error?.message || null,
    backendMessage,
    matchJobs,
    filterJobs,
    refetchJobs,
