...
export function CareerGuideCard({ guide }: CareerGuideCardProps) {
  ...
  const strokeDashoffset =
    circumference - (guide.readiness_score / 100) * circumference;

  // Determine color based on readiness score (matches circular-progress thresholds)
  const getScoreColor = (score: number) => {
    if (score >= 75) return "text-[#05e34f] dark:text-[#04c945]"; // Green
    if (score >= 50) return "text-yellow-500 dark:text-yellow-400"; // Yellow
    if (score >= 30) return "text-orange-500 dark:text-orange-400"; // Orange
    ...
    <HoverCard openDelay={200}>
      ...
      >
        <div className="space-y-3">
          <div>
            <h4 className="text-xs font-medium text-foreground mb-1">Domain</h4>
            <p className="text-xs text-muted-foreground">{guide.domain}</p>
          </div>
          {guide.target_job && (
            <div>
