...
export default function ServiceCard({
  ...
  > = {
    ...
    emerald: {
      ...
      text: "text-emerald-600 dark:text-cyan-300",
    },
    purple: {
      bg: "bg-purple-500/10",
      ...
      <div className="relative overflow-hidden rounded-lg h-[320px] transition-all duration-200 hover:scale-[1.02] cursor-pointer">
        ...
        <div className="relative h-full p-6 flex flex-col">
          <div className="flex items-center justify-between mb-4">
            <div
              className={`inline-flex items-center gap-2 px-2 py-1 rounded-full ${tagCls.bg} border ${tagCls.border}`}
            >
              {TagIcon ? (
                <TagIcon className={`h-3 w-3 ${tagCls.text}`} />
              ) : null}
              <span
                className={`text-[10px] font-medium ${tagCls.text} font-label`}
              >
                {tagText}
              </span>
            </div>
