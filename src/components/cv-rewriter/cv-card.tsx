...
interface CVCardProps { cv: CV }

export function CVCard({ cv }: CVCardProps) {
  // Fix: removed fake "+ 10" inflation and guarded against division by zero
  const scoreImprovement =
    cv.original_score > 0
      ? Math.round(((cv.final_score - cv.original_score) / cv.original_score) * 100)
      : 0;
    ...
    <HoverCard openDelay={200}>
      ...
      >
        <div className="space-y-2">
          <h4 className="text-xs font-medium text-foreground">Job Description Summary</h4>
          <p className="text-xs leading-relaxed text-muted-foreground">{cv.jobs_summary}</p>
        </div>
      </HoverCardContent>
    </HoverCard>
  );
