...
import { X, MapPin, Briefcase, Clock, Award, Building } from "lucide-react";
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-2">
              {JOB_FUNCTIONS.map((func) => (
                <Badge
                  key={func}
                  variant={
                    localFilters.job_functions?.includes(func)
                      ? "default"
                      : "outline"
                  }
            ...
            <div className="flex flex-wrap gap-2">
              {JOB_TYPES.map((type) => (
                <Badge
                  key={type.value}
                  variant={
                    localFilters.job_types?.includes(type.value)
                      ? "default"
                      : "outline"
                  }
                  className="cursor-pointer"
            ...
            <div className="flex flex-wrap gap-2">
              {WORK_MODELS.map((model) => (
                <Badge
                  key={model.value}
                  variant={
                    localFilters.work_models?.includes(model.value)
                      ? "default"
                      : "outline"
                  }
                  className="cursor-pointer"
            ...
            <div className="flex flex-wrap gap-2">
              {EXPERIENCE_LEVELS.map((level) => (
                <Badge
                  key={level.value}
                  variant={
                    localFilters.experience_levels?.includes(level.value)
                      ? "default"
                      : "outline"
                  }
                  className="cursor-pointer"
                  onClick={() =>
                    toggleArrayFilter("experience_levels", level.value)
                  }
                >
                  {level.label}
                </Badge>
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2 mb-2">
              ...
              />
              <Button size="sm" onClick={addCustomLocation}>
                Add
              </Button>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              {localFilters.locations
                ?.filter((loc) => !COMMON_LOCATIONS.includes(loc))
                .map((location) => (
                  <Badge
                    key={location}
                    variant="default"
            ...
            <div className="flex gap-2">
              {[7, 14, 30].map((days) => (
                <Badge
                  key={days}
                  variant={
                    localFilters.posted_within_days === days
                      ? "default"
                      : "outline"
                  }
                  className="cursor-pointer"
