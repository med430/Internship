...
const upload_cv_command_1 = require("../../../Application/Features/CvFeature/Commands/upload-cv.command");
const delete_cv_command_1 = require("../../../Application/Features/CvFeature/Commands/delete-cv.command");
let CVController = class CVController {
    ...
    async upload(file, user) {
        if (!file)
            throw new common_1.BadRequestException('CV required');
        return this.commandBus.execute(new upload_cv_command_1.UploadCVCommand(user.id, file));
    }
    delete(id, user) {
        return this.commandBus.execute(new delete_cv_command_1.DeleteCVCommand(user.id, id));
    }
};
